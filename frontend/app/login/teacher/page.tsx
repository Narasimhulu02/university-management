...
export default function TeacherLoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error?.code) {
        toast.error("Invalid email or password. Please make sure you have already registered as a teacher and try again.");
        setIsLoading(false);
        return;
      }

      // Check if user has teacher role
      if (data?.user && (data.user as any).role !== "teacher") {
        ...
        <CardContent>
          ...
          <div className="mt-4 text-center text-sm">
            <p className="text-muted-foreground">
              ...
              <Link href="/register/teacher" className="text-primary hover:underline font-medium">
                Register here
              </Link>
            </p>
          </div>
